...
Detect takeoff automatically from a flight's stored ADS-B path instead of relying solely on
a manual crew action. Airports carry a boundary polygon (`Airport.shape`), and the backend
already backs up each flight's path; once the recorded path leaves the departure airport's
perimeter, the aircraft has departed. For a flight in `taxiing_out` whose departure airport
has a boundary, the system inspects the stored path on each update and reports takeoff when
the earliest position falls outside the perimeter, backdating `actual.takeoffTime` to that
crossing. Detection is gated by boundary geometry, runs at most once per flight, and produces
the same status transition and `flight.takeoff-reported` event as a manual takeoff, flagged
