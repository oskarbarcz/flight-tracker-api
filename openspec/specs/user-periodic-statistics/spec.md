# user-periodic-statistics

## Purpose

Expose a pilot's time-scoped flight statistics: totals for the current and previous week,
month, and year, the airports and aircraft types first flown within a period, and a per-day
activity heatmap series. Calendar boundaries are UTC (Zulu) with weeks running
Monday–Sunday, and reads are cached with boundary-aware freshness.
