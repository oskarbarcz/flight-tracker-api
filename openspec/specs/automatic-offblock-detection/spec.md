# automatic-offblock-detection

## Purpose

Detect off-block automatically from the stored flight path while a flight is
`boarding_finished`, so a flight no longer depends on a manual crew action to leave the gate.
Each stored position already carries a ground speed (knots); once a boarding-finished aircraft
starts rolling at pushback, its ground speed rises above a walking pace. This capability turns
that observation into an automatic off-block report — backdated to the first moving position,
reported at most once, and flagged as automatically detected — while manual off-block reporting
continues to work identically in every other respect.
