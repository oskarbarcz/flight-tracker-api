# cabin-layout-versions

## Purpose

Store a layout's seat data as immutable revisions so a flight seated against one revision
keeps its seating when the cabin is redrawn. Covers the lazy first fetch, the content hash
that decides whether a refresh writes a new revision, the per-deck coordinate spaces of a
double-decker, and the seat attributes reported by
`GET /api/v1/cabin-layout/{id}/seat-map` and `POST /api/v1/cabin-layout/{id}/refresh`.
