# aircraft-cabin-assignment

## Purpose

Assign one catalogued AeroLOPA cabin layout to an aircraft by hand, and offer ranked
candidates for it. AeroLOPA keys layouts by airline and cabin configuration with no register
of individual aircraft, so the choice is a judgement call operations makes through
`PUT`/`DELETE /api/v1/operator/{operatorId}/aircraft/{aircraftId}/cabin-layout`, helped by
`GET .../cabin-layout/suggestions`. Curated airframes carry the IATA type code the catalogue
is keyed by, alongside their ICAO designator.
