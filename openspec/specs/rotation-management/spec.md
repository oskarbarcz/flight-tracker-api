...
intended times, with block time computed on read) that a real flight is later
attached to, at any point before that flight checks in; the flight remains
ignorant of rotations, and the rotation advances by reacting to existing flight
lifecycle events. Rotation reads are public; every write is gated to the
Operations role.
