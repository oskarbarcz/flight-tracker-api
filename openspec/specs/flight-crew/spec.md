...
Record which crew members are on which flight. The link is established two ways —
automatically when a SimBrief roster is imported, and explicitly by operations
building a flight's manifest from its operator's crew pool — and is exposed as a
flight sub-resource read.
...
The system SHALL allow a user with the `operations` role to add a crew member to a flight via `POST /api/v1/flight/{flightId}/crew` with body `{ crewId }`. The crew member MUST already exist and MUST belong to the flight's operator. The flight MUST NOT have finished boarding. On success the crew member is linked to the flight and appears in the flight's crew list.

#### Scenario: Assign an operator's crew member to its flight

- **WHEN** operations posts a `crewId` of a crew member belonging to the flight's operator, and the flight has not finished boarding
- **THEN** the crew member is linked to the flight and returned in the flight's crew list

#### Scenario: Cannot assign crew after boarding is finished

- **WHEN** operations posts a `crewId` to a flight whose boarding has finished (or a later status)
- **THEN** the request is rejected as unprocessable and no link is created
...
### Requirement: Operations can unassign a crew member from a flight

The system SHALL allow a user with the `operations` role to unassign a crew member from a flight via `DELETE /api/v1/flight/{flightId}/crew/{crewId}`, provided the flight has not finished boarding. Unassigning the link MUST NOT delete the crew record itself.

#### Scenario: Unassign a crew member from a flight

- **WHEN** operations unassigns a crew member from a flight that has not finished boarding
- **THEN** the crew member no longer appears in the flight's crew list
- **AND** the crew member remains listed under its operator

#### Scenario: Cannot unassign crew after boarding is finished

- **WHEN** operations unassigns a crew member from a flight whose boarding has finished (or a later status)
- **THEN** the request is rejected as unprocessable and the link is unchanged

#### Scenario: Non-operations user cannot unassign crew

- **WHEN** a user without the `operations` role unassigns a crew link
- **THEN** the request is rejected as forbidden
