# flight-statistics-accrual

## Purpose

Keep user flight statistics derived from the flight, which is the single source of
truth. Completed flights persist their own aggregatable completion facts, and each
flight-completion event recomputes the affected pilot's projections from those flights
rather than incrementing counters — so accrual is idempotent and always consistent with
source. Covers the historical backfill and the forward `stats-changed` event.
