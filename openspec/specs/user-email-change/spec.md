# user-email-change Specification
...
#### Scenario: A bearer token present on the confirmation changes nothing

- **WHEN** a valid confirmation token is submitted by a caller signed in as another user
- **THEN** the change is applied to the token's account exactly as it would be without a token
