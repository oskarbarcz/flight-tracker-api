# cabin-layout-catalogue

## Purpose

Mirror the AeroLOPA layout index locally so cabin layouts can be browsed and assigned
without calling the provider on every read. Covers `POST /api/v1/cabin-layout/sync`, the
merging of deck-split layouts into one, retirement of layouts withdrawn upstream, and the
catalogue reads at `GET /api/v1/cabin-layout` and `GET /api/v1/cabin-layout/{id}`.
