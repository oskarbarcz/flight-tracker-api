...
Bring a flight's cargo manifest into line with the final loadsheet when boarding finishes,
offloading or adding shipments until the loaded weight equals the final cargo tonnage exactly.
Covers the shipments that stay being left untouched, the ascending offload priority order,
retaining an offloaded shipment with its reason, applying every generation rule to what is
added, and rejecting a final tonnage the hold cannot take.
