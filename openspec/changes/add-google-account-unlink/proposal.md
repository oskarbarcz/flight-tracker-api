...
Linking is also one-way today: `POST /api/v1/auth/google/link` sets `googleId` and
nothing ever clears it. A user who linked the wrong Google account cannot correct it.

## What Changes

- Add `POST /api/v1/auth/password` — an authenticated user whose account has **no**
  ...
  an existing password stays with `PATCH /api/v1/auth/password`.
- Add `POST /api/v1/auth/google/unlink` — an authenticated user, on submitting their
...
- After unlinking, `POST /api/v1/auth/google/link` works again, so a user can move
...
- _None._ `user-password-change` keeps its exact contract: `PATCH /api/v1/auth/password`
...
- **Depends on `add-self-service-password-change`** for `UsersRepository.verifyPassword`,
  `UsersRepository.setPassword`, `SessionService.closeAllForUserExcept`, and
  `PasswordNotSetError`.
- **`auth` module:** `SetPasswordCommand` and `UnlinkGoogleAccountCommand` + handlers;
  `SetPasswordAction` (`POST /api/v1/auth/password`) and `UnlinkGoogleAccountAction`
  (`POST /api/v1/auth/google/unlink`); `SetPasswordDto` and `UnlinkGoogleAccountDto`;
  new errors `PasswordAlreadySetError` and `GoogleAccountNotLinkedError`.
- **`users` module:** `UsersRepository` gains `unlinkGoogleAccount(userId)`, the
  counterpart of the existing `linkGoogleAccount`.
- **No schema/migration change** — `googleId` is already nullable.
- **Functional tests:** new `features/auth/auth.google-unlink.feature` covering the
  full Google-only escape journey, and additions to the Google sign-in feature
  asserting that an unlinked account can no longer sign in with its ID token.
