...
The system SHALL issue a notification to captain when operations releases a flight to the pilot,
describing the load as the preliminary loadsheet and the generated manifest leave it, and SHALL
issue a second one when boarding is finished, describing the load as reconciliation leaves it.
Each notification SHALL be an immutable record of the load at the moment it was issued.

#### Scenario: Release issues the preliminary notification

- **WHEN** operations releases a flight to the pilot
- **THEN** a preliminary notification to captain is issued describing the generated load

#### Scenario: Finishing boarding issues the final notification

- **WHEN** boarding is finished
- **THEN** a final notification to captain is issued describing the reconciled load

#### Scenario: A notification does not change afterwards

- **GIVEN** a flight whose preliminary notification has been issued
- **WHEN** the manifest is later reconciled
- **THEN** the preliminary notification still describes the load as it was at release

### Requirement: A notification is issued whether or not dangerous goods are carried

The system SHALL issue a notification to captain for every released flight, stating explicitly
that no dangerous goods are loaded where none are, rather than issuing nothing.

#### Scenario: A flight with no dangerous goods still has a notification

- **GIVEN** a released flight carrying no dangerous goods
- **WHEN** its notification to captain is read
- **THEN** it is returned, stating that no dangerous goods are loaded

#### Scenario: An unreleased flight has no notification
