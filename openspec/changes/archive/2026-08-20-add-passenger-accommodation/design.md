...
breakdown is: a policy on the loadsheet, run by both the planning and the boarding-completion
commands.
...
The seed carries the whole chain a client needs to draw a cabin, because a manifest cannot be
seeded without one: `cabin_layout_version` rows for `aa-77w`, `de-321` and `lh-74h` are
assembled at seed time from the same provider fixtures the mock serves, through the same
`assembleVersion` the app uses, so their content hash matches and a refresh reports no change.
Cabins are then assigned across the American and Lufthansa fleets — Lufthansa borrows the
`lh-74h` cabin for its A330s, flagged mismatched, because AeroLOPA publishes no A339 layout —
and every released flight gets a manifest generated by the production functions, reconciled
against its final loadsheet where it has one. Twenty-two of thirty-five flights carry a
manifest, with no-shows where boarding closed below the planned count.

Seven fixture flights exist purely so features need no lifecycle lead-in: four Condor flights
awaiting release (a partial load, a full load, an over-capacity load and one with a planned
cabin breakdown), one already checked in with a manifest to read, one boarding to reconcile,
and one Air France flight boarding without any cabin at all. American passenger counts came
down from 370 to 296 in the process: 370 never fitted the 304-seat cabin AeroLOPA publishes
for an American 777-300ER.
