Feature: List recent operators
  ...
  Scenario: Disabling the filter returns the full operator list
    Given I am signed in as "cabin crew"
    When I send a "GET" request to "/api/v1/operator?recent-only=false"
    Then the response status should be 200
    And the response body should contain:
      """json
      [
  ...
  Scenario: As a cabin crew I cannot pass an unsupported filter value
    Given I am signed in as "cabin crew"
    When I send a "GET" request to "/api/v1/operator?recent-only=maybe"
    ...
    And the response body should contain:
      """json
      {
        ...
        "error": "Bad Request",
        "violations": {
          "recent-only": ["recent-only must be a boolean value"]
        }
      }
      """
  ...
  Scenario: As a cabin crew I cannot pass the filter without a value
    Given I am signed in as "cabin crew"
    When I send a "GET" request to "/api/v1/operator?recent-only"
    Then the response status should be 400
    And the response body should contain:
      """json
      {
