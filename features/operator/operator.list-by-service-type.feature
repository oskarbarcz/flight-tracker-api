Feature: Filter operators by service type

  Scenario: As an admin I list the operators carrying freight
    Given I am signed in as "admin"
    When I send a "GET" request to "/api/v1/operator?serviceType=cargo"
  ...
  Scenario: As an admin I list the operators carrying passengers
    Given I am signed in as "admin"
    When I send a "GET" request to "/api/v1/operator?serviceType=passenger"
    Then the response status should be 200
    And the response body should contain:
      """json
      [
  ...
  Scenario: As an admin I list only the operators carrying both
    Given I am signed in as "admin"
    When I send a "GET" request to "/api/v1/operator?serviceType=both"
    Then the response status should be 200
    And the response body should contain:
      """json
      [
  ...
  Scenario: As operations I can filter the operator list
    Given I am signed in as "operations"
    When I send a "GET" request to "/api/v1/operator?serviceType=cargo"
    Then the response status should be 200

  Scenario: As a cabin crew I can filter the operator list
    Given I am signed in as "cabin crew"
    When I send a "GET" request to "/api/v1/operator?serviceType=cargo"
    Then the response status should be 200

  Scenario: As operations my recent carriers are narrowed before they are capped
    Given I am signed in as "operations"
    When I send a "GET" request to "/api/v1/operator?recentOnly=true&serviceType=cargo"
    Then the response status should be 200
    And the response body should contain:
      """json
      [
  ...
  Scenario: As operations I cannot filter by an unsupported traffic kind
    Given I am signed in as "operations"
    When I send a "GET" request to "/api/v1/operator?serviceType=freight"
    ...
    And the response body should contain:
      """json
      {
        ...
        "statusCode": 400,
        "violations": {
          "serviceType": ["serviceType must be one of the following values: passenger, cargo, both"]
        }
      }
      """

  Scenario: As operations the former hyphenated spelling of the filter is not accepted
    Given I am signed in as "operations"
    When I send a "GET" request to "/api/v1/operator?service-type=cargo"
    Then the response status should be 400
    And the response body should contain:
      """json
      {
        ...
        "statusCode": 400,
        "violations": {
          "service-type": ["property service-type should not exist"]
        }
      }
      """

  Scenario: As an unauthorized user I cannot filter the operator list
    When I send a "GET" request to "/api/v1/operator?serviceType=cargo"
