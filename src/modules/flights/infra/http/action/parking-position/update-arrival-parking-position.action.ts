...
import {
  GetFlightResponse,
  UpdateArrivalParkingPositionRequest,
...
import { UpdateArrivalParkingPositionCommand } from '../../../../application/command/update-arrival-parking-position.command';
...
export class UpdateArrivalParkingPositionAction {
  constructor(
  ...
  @ApiOperation({
    summary: 'Update flight arrival parking position',
    description:
      'Assigns the arrival parking position to a flight. <br />' +
  ...
  @ApiBody({ type: UpdateArrivalParkingPositionRequest })
  @ApiOkResponse({ type: GetFlightResponse })
  @ApiBadRequestResponse({ type: GenericBadRequestResponse })
  @ApiUnauthorizedResponse({ type: UnauthorizedResponse })
  @ApiForbiddenResponse({ type: ForbiddenResponse })
  @ApiNotFoundResponse({ type: GenericNotFoundResponse })
  @Patch('/:id/arrival-parking-position')
  @Role(UserRole.Operations, UserRole.CabinCrew)
  async run(
    @UuidParam('id') id: string,
    @Req() request: AuthorizedRequest,
    @Body() body: UpdateArrivalParkingPositionRequest,
    ...
    const assertCommand = new AssertParkingPositionBelongsToAirportCommand(
      arrivalAirportId,
      body.arrivalParkingPositionId,
    );
    await this.commandBus.execute(assertCommand);

    const updateCommand = new UpdateArrivalParkingPositionCommand(
      id,
      request.user,
      body.arrivalParkingPositionId,
    );
