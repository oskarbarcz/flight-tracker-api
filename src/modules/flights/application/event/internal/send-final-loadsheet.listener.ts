...
@Injectable()
export class SendFinalLoadsheetListener {
  private readonly logger = new Logger(SendFinalLoadsheetListener.name);
  ...
  public async onBoardingFinished(
    event: BoardingWasFinishedEvent,
  ): Promise<void> {
    ...
    try {
      ...
      const recipientQuery = new GetDiscordRecipientQuery(
        captainId,
        DiscordNotification.FinalLoadsheet,
      );
      ...
      await this.client.sendDirectMessage(discordId, {
        flightId,
        content,
        type: 'final-loadsheet',
      });
    } catch (error) {
      this.logger.warn(
        `Could not send final loadsheet to Discord for flight ${flightId}: ${getErrorMessage(error)}`,
      );
    }
  }
}
