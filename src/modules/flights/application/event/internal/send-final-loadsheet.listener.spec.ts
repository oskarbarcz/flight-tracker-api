import { SendFinalLoadsheetListener } from './send-final-loadsheet.listener';
...
function boardingFinished() {
  return new BoardingWasFinishedEvent({
    flightId: FLIGHT_ID,
...
describe('SendFinalLoadsheetListener', () => {
  ...
  beforeEach(() => {
    ...
    queryBus = {
      execute: jest.fn().mockImplementation((query: unknown) => {
        if (query instanceof GetDiscordRecipientQuery) {
          expect(query.notification).toBe(DiscordNotification.FinalLoadsheet);
        ...
        if (query instanceof GetCurrentLoadsheetQuery) {
          expect(query.kind).toBe(LoadsheetKind.Final);
    ...
    listener = new SendFinalLoadsheetListener(
      client as never,
      queryBus as never,
  ...
  it('sends the final loadsheet to the captain', async () => {
    await listener.onBoardingFinished(boardingFinished());
    ...
    expect(message.type).toBe('final-loadsheet');
    expect(message.flightId).toBe(FLIGHT_ID);
    expect(message.content).toContain(
      ':clipboard: **Flight LH 81 final loadsheet**',
    );
  ...
  it('sends nothing when the flight has no final loadsheet', async () => {
    final = null;
  ...
  it('keeps a failing message from breaking the boarding finish', async () => {
    client.sendDirectMessage.mockRejectedValue(new Error('discord is down'));

    await expect(
      listener.onBoardingFinished(boardingFinished()),
